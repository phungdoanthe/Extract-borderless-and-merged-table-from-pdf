...
# compare with level of similarity
def compare_table_cells_by_similarity(diff_cells, pdf_names):
    ...
    for col in diff_cells.filter(regex='spec_value').columns.tolist():
        if diff_cells[col].astype(str).str.contains("missing", case=False, na=False).any():
            compare_criteria = diff_cells.columns.tolist().index('indicator')+1
            common_cols = diff_cells.columns.tolist()[:compare_criteria]
        else:
            return diff_cells
    ...
    for i in range(1, len(pdf_names)):
        ...
        # make sure two df is not have same columns
        pdf_df1 = pdf_df1[[col for col in pdf_df1.columns if not col.endswith(pdf_name2)]]
        pdf_df2 = pdf_df2[[col for col in pdf_df2.columns if not col.endswith(pdf_name1)]]
        
        if pdf_df1['contents'].iloc[0] != pdf_df2['contents'].iloc[0]:
            join_cols = common_cols[1:]
...
def compare_pdf(pdf_list, host=host, port=port, database=dbname_ERS, username=username, password=password):
    ...
    for pdf in pdf_list:
        ...
        #list in list
        database_name.append(schema)
    #compare
    no_table = [len(schema) for schema in database_name]
    #find the schema has the greates number of tables
    longest_schema = [schema for schema in database_name if len(schema) == max(no_table)][0]
    compared_table_list = []
    #extract table from db based on table name
    notifications = []
    for table_name in longest_schema:
        table_list = []
